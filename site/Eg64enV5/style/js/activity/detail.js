...
require(['/common/js/require.config.js'], function () {
	require(['jquery', 'vue', 'dic', 'httpVueLoader',
		'./style/js/api/activity.js', 'httpUrl','fileSaver'],
		function ($, Vue, dic, httpVueLoader, indexApi, httpUrl,fileSaver) {
			new Vue({
				el: '#index_box',
				data: {
					breadcrumb: [
				...
				methods: {
					...
					getRecommendById: function (id) {
						...
						indexApi.getRecommendActiveById({ id: id }).then(function (res) {
							res.result && res.result.forEach(function (item) {
								item.itemUrl = './activityDetail.html?id=' + item.id
								item.styles = {
					...
					getEvaluateById: function (id, page) {
						var vm = this
						page = page || 1;
						indexApi.getActiveEvaluateListByPage({ id: id || this.detail.id, pageNum: 1, pageSize: 10000 }).then(function (res) {
					...
					getselectIssueMonth: function () {
						var vm = this
						indexApi.selectIssueMonth({ year: vm.calendar.year, month: vm.calendar.month + 1 }).then(function (res) {
					...
					getselectIssueToday: function () {
						var vm = this
						indexApi.selectIssueToday({ pageNum: 1, pageSize: 100, activeNowTime: this.activeNowTime }).then(function (res) {
							res.result && res.result.list.forEach(function (item) {
								item.itemUrl = './activityDetail.html?id=' + item.id
							});
						...
						if (this.detail.onLineFlag === '1'&&this.detail.liveFlag.toString()==='1'&&this.detail.isThird.toString()==='1'&&this.detail.pullStreamUrl) {
							...
							if (this.userInfo && this.userInfo.userName) {
								if (this.detail.statusCode === "0302") {
									window.location.href = './activitySignUp.html?id=' + this.detail.id
								}else{
