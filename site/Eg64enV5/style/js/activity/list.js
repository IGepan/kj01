...
require(['/common/js/require.config.js'], function () {
    require(['jquery', 'vue', 'dic', 'httpVueLoader',
            './style/js/api/activity.js', 'httpUrl', 'laydate',],
                        ...
                        indexApi.selectIssuePage(this.searchForm).then(function (res) {
                            res.result && res.result.list.forEach(function (item) {
                                ...
                                if (Array.isArray(item.activeTypeDisplay) && item.activeTypeDisplay.length) {
                                    item.activeTypeDisplay = item.activeTypeDisplay[0].name ? item.activeTypeDisplay[0].name : item.activeTypeDisplay[0].objName
                                } else {
                        ...
                        this.$httpCom.dictList({dictInfos: keys}).then(function (res) {
                            if (res.code === 'rest.success') {
                                ...
                                opts = res.result.map(function (codes, i) {
                                    ...
                                    codes.code !== 'sort_type' ? codes.dictIInfos.unshift({
                                        id: "-1",
                                        value: -1,
                                        display: codes.code === 'yes_no' ? '不限' : '全部',
                                        selected: true
