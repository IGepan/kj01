...
require(['/common/js/require.config.js'], function () {
	require(['jquery', 'vue', 'dic', 'httpVueLoader',
		'./style/js/api/index.js', 'httpUrl', 'laydate','./style/js/libs/pagination.js'],
		function ($, Vue, dic, httpVueLoader, indexApi, httpUrl,laydate,pagination) {
			new Vue({
				...
				methods: {
					...
					getBox1List:function(){
						let vm=this
						indexApi.getEnterList(this.params1).then(function(res) {
							if (res.code === "rest.success") {
								vm.dataList = res.result.list;
								vm.count=res.result.total
								// vm.page1=res.result.pageNum
								vm.$data.pages = res.result || ''
							}
						});
					},
					getHighList:function() {
						let vm=this,
							params={
								pageNum: 1,
								pageSize: 10,
						...
						indexApi.getEnterList(this.params2).then(function(res) {
							if (res.code === "rest.success") {
								vm.dataList = res.result.list;
								vm.count=res.result.total
								// vm.page1=res.result.pageNum
