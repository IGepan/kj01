...
require([baseUrlPath + '/common/js/require.config.js'], function () {
  require(['jquery', 'vue', 'dic', 'httpVueLoader', 'userCenter', 'httpUser', '/style/js/api/vote.js', 'jqValidate', 'dialog', 'httpUrl', 'laydate', 'jqSelect', 'VueUeditor'],
   function ($, Vue, dic, httpVueLoader, userCenter, httpUser, voteApi, jqValidate, dialog, httpUrl, laydate, jqSelect, VueUeditor) {
            ...
            if (res.code === 'rest.success') {
              ...
              for (var key in to) {
                if (key !== 'poster') {
                  if (key === 'sponsor') {
                    // 初始化 主办方
                    vm.formData[key] = to[key].split('ぶんかつ');
