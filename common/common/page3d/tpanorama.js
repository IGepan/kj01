...
(function (global, undefined) {
	...
	tpanorama.prototype = {
		...
		render: function render(opt) {
			...
			initCamera(this.def);
			initRaycaster();
			makePanorama(this.def.pRadius, this.def.widthSegments, this.def.heightSegments, this.def.url, this.def.startAngle);
			initRenderer(this.def);
			initLable(this.def.lables, this.def.sprite, this.def);
	...
	function initCamera(def) {
		if (def.width && def.height) {
			_camera = new THREE.PerspectiveCamera(_fov, def.width / def.height, 1, 1100);
			_camera.target = new THREE.Vector3(0, 0, 0);
			_cameraOrtho = new THREE.OrthographicCamera(-def.width / 2, def.width / 2, def.height / 2, def.height / 2, 1, 10);
		} else {
	...
	function makePanorama(pRadius, widthSegments, heightSegments, u, startAngle) { //切段
		var mesh = new THREE.Mesh(new THREE.SphereGeometry(pRadius, widthSegments, heightSegments,startAngle), new THREE.MeshBasicMaterial({
	...
	function initRenderer(opts) {
		...
		_renderer.setSize(opts.width || window.innerWidth, opts.height || window.innerHeight);
	...
	function onDocumentMouseClick(event) {
		...
		if (this.def.width && this.def.height) {
			_mouse.x = event.clientX / this.def.width * 2 - 1;
			_mouse.y = -(event.clientY / this.def.height) * 2 + 1;
		} else {
	...
	function initLable(lables, sprite, def) {
		if (sprite == 'label') {
...
//				newPosition.lon = def.width && def.height ? def.width / window.innerWidth * lables[i].position.lon : lables[i].position.lon;
//				newPosition.lat = def.width && def.height ? def.height / window.innerHeight * lables[i].position.lat : lables[i].position.lat;
				newPosition.lon = lables[i].position.lon+(def.startAngle/(Math.PI * 2)*360);
				newPosition.lat = lables[i].position.lat;
				_sprites.push(createSprite(newPosition, lables[i].logoUrl, lables[i].text,lables[i].uid));
			}
		}
	}
	...
	function createSprite(position, url, name,id) {
	...
	function worldPostion2Screen(world_vector, camera, def) {
		var vector = world_vector.clone();
		vector.project(camera);
		if (def.width && def.height) {
			var result = {
				x: Math.round((vector.x + 1) * def.width / 2 - def.width / 2),
				y: Math.round(def.height / 2 - (-vector.y + 1) * def.height / 2),
				z: 0
			};
		} else {
			var result = {
