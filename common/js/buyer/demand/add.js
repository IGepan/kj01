      ...
      methods: {
        ...
        customarea: function (v, o, callback, e) {
          ...
          if (index !== undefined && index > 1) {
            ...
            var vText = $.map($('.iptbox .expslt'), function (sel) { return $(sel).find("option:selected").text() })
            if (v.filter(function (i) { return i }).length >= 2) {
              callback(o)
            } else {
              callback(o, '面向地区必选')
            }
        ...
        createdInfos: function (data) {
          ...
          html += '<tr><th>所属行业：</th><td><div class="spans">' + data.industryList.map(function (i) { return '<span>' + i.name + '</span>' }).join('') + '</div></td></tr>'
          html += '<tr><th>需求类型：</th><td>' + $('.inputradio input:checked').next().text() + '</td></tr>'
          data.keyWord && (html += '<tr><th>标签：</th><td><div class="spans">' + this.dataForm.keyWord.map(function (i) { return '<span>' + i + '</span>' }).join('') + '</div></td></tr>')
          html += '<tr><th>面向地区：</th><td>' + ($.map($('.iptbox .expslt'), function (sel) { return $(sel).find("option:selected").text() }).join(' ')) + '</td></tr>'
          ...
          data.quotaList.length && (html += '<tr><th style="font-weight: 700">投标指标（自定义）：</th><td></td></tr>' + data.quotaList.map(function (i) { return '<tr><th>' + i.quotaName + '：</th><td>' + i.quotaComment + '</td></tr>' }).join(''))
          data.fileList.length && (html += '<tr><th>需求附件：</th><td><ul>' + data.fileList.map(function (i) { return '<li>' + i.name + '</li>' }).join('') + '</ul></td></tr>')
