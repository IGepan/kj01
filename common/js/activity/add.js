...
require([baseUrlPath + '/common/js/require.config.js'], function () {
  require(['jquery', 'vue', 'dic', 'httpVueLoader', 'userCenter', 'httpUser', '/common/js/httpApi/activity.js', 'jqValidate', 'dialog', 'httpUrl', 'laydate', 'jqSelect', 'VueUeditor'], function ($, Vue, dic, httpVueLoader, userCenter, httpUser, activityApi, jqValidate, dialog, httpUrl, laydate, jqSelect, VueUeditor) {
      ...
      methods: {
        ...
        getTemplateList: function () {
          ...
          activityApi.selectPolicy({}).then(function (res) {
            if (res.code === 'rest.success') {
              res.result.forEach(item => { item.selected = 0 })
          ...
          $('.valiform').validate('submitValidate', function (val) {
            if (val) {
              ...
              activityApi[type](data).then(function (res) {
                if (res.code === 'rest.success') {
                  setTimeout(() => {
                    location.href = this.$pathPrefix+'/common/activity/list.html?code=001.004.001.001'
                  }, 2000);
                } else {
