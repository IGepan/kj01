    ...
    window.vueDom = new Vue({
      ...
      methods: {
          ...
          $('.orderValiform').validate('submitValidate', function (val) {
            ...
            if (val) {
              if (!vm.addressList.length) {
                ...
                return
              } else if (vm.selecedAddressIndex === -1 && !vm.addressList.some(function (add) { return add.defaultFlag === '1' })) {
                vm.$dialog.showToast('请选择委托人信息！')
                return
              }
              contactsId = vm.selecedAddressIndex === -1 ? vm.addressList.filter(function (add) { return add.defaultFlag === '1' })[0].id : vm.addressList[vm.selecedAddressIndex].id
              !vm.isSubmitDisabled && (vm.isSubmitDisabled = true, httpCartApi.getFormToken().then(function (res) {
                if (res.code == 'rest.success') {
                  ...
                  httpCartApi.addOrder({ contactsId: contactsId, formToken: res.result, orders: vm.shopList, diagnosis: diagnosis }).then(function (res) {
                    if (res.code == 'rest.success') {
        ...
        addressSetDefault: function (address) {
          var vm = this
          httpUserAddressApi.addressSetDefault({ id: address.id, version: address.version }).then(function (res) {
            if (res.code == 'rest.success') {
              vm.getAddressList()
            }
          })
